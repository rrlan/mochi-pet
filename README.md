...
> 🐈 **想直接用上面这只猫？** 它叫**杨百乐**（[@rrlan](https://github.com/rrlan) 的猫），整套形象包在 [`appearances/yangbaile/`](appearances/yangbaile/)。两种装法：
>
> - **菜单（推荐）**：🍡 → 形象 → **导入形象包…** → 选 `appearances/yangbaile/` 文件夹，含走路帧、即时生效
> - **命令**：`mkdir -p ~/.mochi/appearances && cp -R appearances/yangbaile/. ~/.mochi/appearances/ && pkill -x Mochi`
...
- **导入形象包…**：选一个形象包**文件夹**（含 5 状态图 + 可选 `walk/`），一键装好整套、含走路帧、即时切换。
- **导入形态图片…**：选一张或多张**图片**，按文件名关键词匹配槽位（否则按顺序填入）；这种**不含走路帧**。

图片都存到 `~/.mochi/appearances/`。
